...
We have changed the policy. detailFlags can now be added, however if detailFlags are added then the maximum number of entities returned is lowered from 100 to (30 - (details.count * 2)).
...
Many API calls have their own Rate limiters as well, the first request exceeding the limit will be held waiting for the time limit to pass before being processed (so the call will not fail, only be slow), subsequent requests before processing the last (when you are calling the API in a multithreaded fassion) will fail with a HTTP 429 "Too many requests" response.
