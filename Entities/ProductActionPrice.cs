...
namespace VenditPublicSdk.Entities
{
    ...
    [Serializable]
    public class ImportProductActionPrice
    {
        public int?       ActionPriceId        { get; set; }
        public int?      ProductId            { get; set; }
        public int?      GroupId              { get; set; }
        ...
        public override string ToString()
        {
            ...
            if (!string.IsNullOrWhiteSpace(BrandName))
                sb.Append(BrandName).Append(" ");
            sb.Append(ActionPriceGroupName).Append(" ");
            if (SalesPriceExVat.HasValue && SalesPriceExVat > 0)
                sb.Append(SalesPriceExVat.Value.ToString("N2"));
            if (DiscountPercentage.HasValue && DiscountPercentage > 0)
                sb.Append(DiscountPercentage.Value.ToString("N2")).Append('%');
